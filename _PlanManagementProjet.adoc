...
== Les outils en soutien à la gestion de projet
...
=== Git et Github
    ...
    GIT005 : Le workflow https://githubflow.github.io/[_Github flow_] doit être utilisé pour la gestion des contributions des différents membres du projet. Les autres bonnes pratiques Git comme la bonne utilisation de fichiers ``.gitgnore`` doivent être mobilisées.
...
=== Google Drive
    ...
    Nom du dossier  : SAE XX (No du projet)  - Intitulé du projet de la SAE - S4.
...
WARNING: Lieu de stockage reste à définir.
...
==== La documentation 
    ...
    - d'une description des modèles de données utilisés,
    - d'une description des technologies utilisées,
    - d'une description de la stratégie de tests (taux de couverture du code, répartition tests unitaires/tests intégration) et de la stratégie du maintien de la qualité du code avec PHPStan,
...
IMPORTANT: La documentation technique sera évaluée sur sa capacité à permettre à un nouvel arrivant sur le projet de comprendre et de s'approprier la solution sur le plan technique.
    ...
    DOC004: Le mode d’emploi pour la configuration de VirtualBox (si besoin) et l’installation des machines virtuelles.
...
==== Suivi des temps individuels
    ...
    - Temps totalisé par étudiant, par sprint et global sur le semestre (S4) avec graphiques.
